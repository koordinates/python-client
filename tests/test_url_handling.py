...
def test_sets_multi_url(client):
    assert client.get_url('SET', 'GET', 'multi') == 'https://koordinates.com/services/api/v1/sets/'


def test_layers_url(client):
    assert client.get_url('LAYER', 'GET', 'single', {'id':999}) == 'https://koordinates.com/services/api/v1/layers/999/'


def test_layers_multi_url(client):
    assert client.get_url('LAYER', 'GET', 'multi') == 'https://koordinates.com/services/api/v1/layers/'


def test_layer_versions_url(client):
    assert (
        client.get_url('VERSION', 'GET', 'single', {'layer_id': 1494, 'version_id': 4067})
